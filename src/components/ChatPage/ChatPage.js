...
import {
  SOCKET_READY_STATE, MESSAGE,
  sendMessageAction, disconnectAction,
...
const ChatPage = () => {
  ...
  const { state: { username }, chat: { close, error, readyState, users, messages } } = store
  const isSocketOpen = !close && !error && readyState === SOCKET_READY_STATE.OPEN

  if (!username || !isSocketOpen) return <Redirect to={{ pathname: '/', state: { from: '/chat' } }} />
  ...
  return (
    <div className={styles.chat}>
      ...
      <div className={styles.rightContainer}>
        ...
        <Input disabled={!isSocketOpen} sendMessage={sendMessage} disconnect={disconnect} />
      </div>
    </div>
  )
}
...
const UserList = ({ active, users, username }) =>
  <ul className={`${styles.userList} ${active ? styles.active : ''}`}>
    {users.map(user => <User key={user} user={user} current={user === username} />)}
  </ul >

const User = ({ user, current }) =>
  <li className={current ? styles.userCurrent : ''}>
    <span className={styles.username}>{user}</span> {current ? <span>(You)</span> : null}
  </li>
...
const Message = ({ message }) => {
  ...
  switch (message.type) {
    ...
    case MESSAGE.USER_DISCONNECTED: return (
      <li className={styles.statusMessage}>
        [{date}] {message.message}
      </li>
    )
    case MESSAGE.USER_MESSAGE: return (
      <li className={styles.userMessage}>
        [{date}] {`<`}{message.username}{`>`}: {message.message}
      </li>
    )
    case MESSAGE.SERVER_MESSAGE: return (
...
const formatDate = dateStr => {
  ...
  return `${date.toLocaleTimeString()}` // `${date.toLocaleTimeString()}` ${date.toLocaleDateString()}`
}
...
export default () => <ErrorBoundary><ChatPage /></ErrorBoundary>